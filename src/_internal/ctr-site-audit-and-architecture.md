...
It immediately launches into "The Problem is Hostility" — which is good theology but confusing rhetoric for a newcomer who doesn't yet know _who these people are_. The visitor doesn't have enough context to evaluate the argument.
...
Every page should answer: _Who are you, what do you do, and why does it matter?_ — in that order, for a first-time visitor. Regulars can navigate deeper.
...
John 17 is the _theological foundation_ of CTR, not a navigation destination. It belongs as the first page under About, where newcomers will encounter it after they've been oriented. The nav label "John 17" assumed prior knowledge.
...
- **Core Beliefs page** — should explicitly state Trinitarian orthodoxy, affirmation of Scripture, and the ecumenical breadth _up front_, not buried
...
```js
eleventyConfig.addCollection('dailyPrayer', (col) => col.getFilteredByTag('daily-prayer').sort((a, b) => b.date - a.date));

eleventyConfig.addCollection('teachingOfWeek', (col) => col.getFilteredByTag('teaching-of-week').sort((a, b) => b.date - a.date));

eleventyConfig.addCollection('bookOfMonth', (col) => col.getFilteredByTag('book-of-month').sort((a, b) => b.date - a.date));
